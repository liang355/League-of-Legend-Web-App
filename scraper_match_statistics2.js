...
var addChampionStatistics = function(championIdentification, championStatistics, thetimeline, summonerID, summonerName, currTier){
    var err = false;
    ...
    try{
        SummonerNew.findOne({'sID':summonerID}, function(err, summoner){

            console.log(printLog + "adding stats for: " + name + ", " + role);
            var champStatistics = new ChampionStatisticsNew();


            champStatistics.id = id;
            champStatistics.name = name;
            champStatistics.role = role;
            champStatistics.summonerID = summonerID;

            if(summoner == null){
                console.log(printLog +"---summoner:"+ summonerID+" does not exsist.");
                champStatistics.tier = summonerID;
                SummonerNew.create({'sID':summonerID, 'name':summonerName, 'tier':404, 'matchTier':currTier});
            }
...
//(3) add stats to DB
var storeStats = function(stats, currTier) {
    ...
    try {
        ...
        for (p = 1; p < 11; p++) {
            ...
            addChampionStatistics(championIdentification, championStatistics, timeline, summonerID, summonerName, currTier);
